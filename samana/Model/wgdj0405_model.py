...
class WGDJ0405ModelEPLShear(_WGDJ0405ModelBase):

    def __init__(self, data_class, kde_sampler=None, shapelets_order=None):
        super(WGDJ0405ModelEPLShear, self).__init__(data_class, kde_sampler, shapelets_order)

    def setup_lens_model(self, kwargs_lens_macro_init=None, macromodel_samples_fixed=None):

        lens_model_list_macro = ['EPL', 'SHEAR']
        kwargs_lens_macro = [{'theta_E': 0.7043503589365834, 'gamma': 2.054987670532628, 'e1': -0.03728206090558943,
                              'e2': 0.14186781405207996, 'center_x': 0.009601035223083986,
                              'center_y': -0.06606995853487262},
                             {'gamma1': 0.010304292416589459, 'gamma2': 0.026736939313574157}]
        ...
        kwargs_lower_lens = [
            {'theta_E': 0.05, 'center_x': -10.0, 'center_y': -10.0, 'e1': -0.5, 'e2': -0.5, 'gamma': 1.5},
            {'gamma1': -0.5, 'gamma2': -0.5}]
        kwargs_upper_lens = [
            {'theta_E': 5.0, 'center_x': 10.0, 'center_y': 10.0, 'e1': 0.5, 'e2': 0.5, 'gamma': 3.5},
            {'gamma1': 0.5, 'gamma2': 0.5}]
...
class WGDJ0405ModelEPLBoxyDiskyShear(_WGDJ0405ModelBase):

    def __init__(self, data_class, kde_sampler=None, shapelets_order=None):
        super(WGDJ0405ModelEPLBoxyDiskyShear, self).__init__(data_class, kde_sampler, shapelets_order)

    @property
    def prior_lens(self):
        return [[0, 'gamma', 2.0, 0.2], [0, 'a4_a', 0.0, 0.01]]

    def setup_lens_model(self, kwargs_lens_macro_init=None, macromodel_samples_fixed=None):

        lens_model_list_macro = ['EPL_BOXYDISKY', 'SHEAR']
        kwargs_lens_macro = [{'theta_E': 0.7043503589365834, 'gamma': 2.0, 'e1': -0.01,
                              'e2': 0.07, 'center_x': 0.009601035223083986,
                              'center_y': -0.06606995853487262, 'a4_a': 0.0},
                             {'gamma1': 0.010304292416589459, 'gamma2': 0.026736939313574157}]
        redshift_list_macro = [self._data.z_lens, self._data.z_lens]
        index_lens_split = [0, 1]
        if kwargs_lens_macro_init is not None:
            for i in range(0, len(kwargs_lens_macro_init)):
                for param_name in kwargs_lens_macro_init[i].keys():
                    kwargs_lens_macro[i][param_name] = kwargs_lens_macro_init[i][param_name]
        kwargs_lens_init = kwargs_lens_macro
        kwargs_lens_sigma = [{'theta_E': 0.05, 'center_x': 0.1, 'center_y': 0.1, 'e1': 0.2, 'e2': 0.2,
                              'gamma': 0.1, 'a4_a': 0.01},
                             {'gamma1': 0.05, 'gamma2': 0.05}]
        kwargs_lens_fixed = [{}, {'ra_0': 0.0, 'dec_0': 0.0}]
        kwargs_lower_lens = [
