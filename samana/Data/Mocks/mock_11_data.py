...
from samana.Data.ImageData.MockImageData.mock_11_simple import image_data as simple_image_data
from samana.Data.ImageData.MockImageData.mock_11_cosmos import image_data as cosmos_image_data
from samana.Data.ImageData.MockImageData.mock_11_cosmos_wdm import image_data as cosmos_image_data_wdm
from samana.Data.ImageData.MockImageData.mock_11_cosmos_highSNR import image_data as cosmos_image_data_highSNR
...
class Mock11DataHighSNR(MockBase):
    ...
    def __init__(self, super_sample_factor=1.0, cosmos_source=True):
        ...
        x_image = [0.98077, -0.33796, 0.65136, -0.54446]
        y_image = [-0.49237, 1.0404, 0.74046, -0.51778]
        magnifications_true = [3.81907, 4.20005, 4.4265, 1.67906]
        magnification_measurement_errors = 0.0
        magnifications = np.array(magnifications_true) + np.array(magnification_measurement_errors)
        astrometric_uncertainties = [0.003] * 4
        flux_ratio_uncertainties = None
        ...
        if cosmos_source:
            ...
            raise Exception('only cosmos source implemented for this class')
        super(Mock11DataHighSNR, self).__init__(z_lens, z_source, x_image, y_image,
                                magnifications, astrometric_uncertainties, flux_ratio_uncertainties,
                                image_data, super_sample_factor)
